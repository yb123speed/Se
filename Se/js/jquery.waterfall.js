...
(function ($, window, document) {
    $.fn.waterfall = function (options) {
                    ...
                    if (jsonCache.length > 0) {
                        ...
                        if (opts.ajaxTimes === 'infinite' || ajaxTimes < opts.ajaxTimes) {
                            ...
                            ajaxFunc(
								function (jsonData) {
								    try {
								        if (typeof jsonData === 'string') jsonData = $.parseJSON(jsonData.table);
								        if ($.isEmptyObject(jsonData.table) || typeof jsonData.table === 'string') {
								            showMsg('finish');
								        } else {
								            //json数据处理
								            jsonCache = jsonCache.concat(jsonData.table).reverse();
								            dealData();
								        }
								    }
								    catch (e) {
        ...
        function dealData() {
            ...
            loadImg(jsonCache, opts.imgUrlName, function () {
                while (perNum-- > 0 && (data = jsonCache.pop())) {
                    ...
                    wf_item_left = minColsIndex * ($wf_box.width() / 2 + opts.marginLeft);
                    ...
                    $wf_item = $('<div>').addClass(opts.itemClass).html(htmlStr)
								.css({ width: opts.itemwidth ? opts.itemwidth : "100%", left: wf_item_left, top: wf_item_top })
								.appendTo($wf_col);
                    $wf_img = $wf_item.find('.' + opts.imgClass);
                    $wf_img.height($wf_img.width() / opts.imageScale);
        ...
        function realign() {
            ...
            colNum = Math.floor(($wf_box.width() + opts.marginLeft) / ($wf_box.width() / 2 + opts.marginLeft));
            //if (colNum > 0 && colNum !== opts.colNum) {
            opts.colNum = opts.initcoln;
            ...
            $wf_col_items.each(function (num, value) {
                ...
                $(this).width(opts.itemwidth ? opts.itemwidth : "100%").animate({
                    left: wf_item_left,
                    top: wf_item_top
                }, speed);
        ...
        // 显示结果信息
        function showMsg(type, finishContent) {
            switch (type) {
                case 'loading':
                    isLoading = true;
                ...
                case 'finish':
                    ...
                    if (finishContent != null) {
                        $wf_result.removeClass('wf_loading').show().html(finishContent);
                    }
                    else {
                        //$wf_result.removeClass('wf_loading').show().html('');
                        $wf_result.removeClass('wf_loading').hide().html('');
                    }
                    isFinish = true;
                    break;
            }
        ...
        return this.each(function () {
            ...
            //			$wf_col.css("margin", "0px 8px");
            $wf_result = $('<div style="float: left; display: block;margin: 0 auto;text-align: center;width: 100%;color: #333;margin-bottom: 35px;">').addClass('wf_result').appendTo($wf_box);
    ...
    $.fn.waterfall.defaults = {
        ...
        postbackRemark: null, //回调处理
        imageScale: 1, //图片比例
        finishContent: null, //完成加载内容
        itemwidth: undefined,
        initcoln: 1
